...
export const UpdateCompany = ({ id, name, indepted }) => {
  ...
  const handleUpdate = async () => {
    await fetch(`http://127.0.0.1:8000/finance/companies/${id}/`, {
      method: "PUT",
      headers: {
      ...
      if (res.ok) {
        handleClose();
        toast.success("تم تحديث اسم الشركه");
    ...
    <>
      ...
      <Dialog open={open} onClose={handleClose}>
        <Box padding={"3rem"} sx={{ direction: "rtl" }}>
          <Box fontWeight={600}>تعديل شركه</Box>
